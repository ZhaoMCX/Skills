...
Use `af-dispatch-agents` for independent review sidecars when the changed scope is large, risky, or benefits from parallel scrutiny.

- Use `reviewer` for correctness, regression risk, AF boundary, and test-gap review.
- Use `security_auditor` for permission, injection, secrets, auth, sandbox, or external boundary risk.
- Use `performance_profiler` for hot paths, repeated work, memory, rendering, network, or database performance risk.

The main agent must still lead with final findings ordered by severity. Sidecar results are evidence, not the final review.
