...
- Do not force AF onto non-development tasks such as image generation, spreadsheets, documents, general web research, pure Q&A, generic delegation, or generic handoff unless the user explicitly asks.
...
- When AF work needs subagents, use the general `dispatch-agents` skill and scope tasks by Primary Module, related Modules, AF element, Write Scope, and verification Gates.
...
- When AF work must continue in another session or by another agent, use the general `handoff` skill and include the active `XXXAgentFramework`, AF Feature Plan, Module context, concrete AF Element progress, and suggested next AF skill.
...
| Delegate independent AF work to subagents | `dispatch-agents` |
...
| Hand work to a future session or another agent | `handoff` |
...
- Subagents are general collaboration tools; in AF work, coordinate them by AF ownership and non-overlapping write scope. Their results never replace main-agent integration, AF review, or final verification.
- Handoffs are general continuation tools; in AF work, reference durable AF artifacts instead of duplicating them and include the next suggested AF skill.
