...
- Use a single foreground DevTools session for the target project. Before switching generated project directories or AppIDs, run official `quit`, wait for DevTools processes from the install directory to exit, and confirm the intended automation port is not still listening.
...
- Treat interactive DevTools dialogs as a separate setup stage. For trust-project, AppID-change, tourist-mode, login, authorization, permission, or security prompts, either make the safe local confirmation in the visible foreground window and report it, or stop and ask the user to handle it. Do not keep running route assertions behind a prompt.
